...
//metodos set
	...
	public function getActaCandidatos(){
		$jornada = $this->getFidjor();
		$idcen = $this->getFidcen();
		$sql = "SELECT u.idusu, u.ndocusu, u.nomusu, f.idfic, f.nomfic, c.nomcen, u.actusu, u.fotcan, v.nomval, 
				(SELECT count(idusu) FROM voto WHERE canusu = u.idusu) as nvo
				FROM usuario AS u 
				...
				ORDER BY (CASE WHEN u.ndocusu = '999' OR u.nomusu LIKE '%BLANCO%' THEN 1 ELSE 0 END) ASC, nvo DESC;";
		$modelo = new conexion();
		$conexion = $modelo->get_conexion();
		$result = $conexion->prepare($sql);
		$result->bindParam(":jornada", $jornada);
