...
if ($fidcen) {
    $dfichas = $votmctele->getFichasByCentro($fidcen);
    $configCentro = $votmctele->getConfiguracionCentro($fidcen);
    $periodoVotacion = $votmctele->verificarPeriodoVotacion($fidcen, 'votacion');
}

if ($fidfic) {
    $datRep = $votmctele->getCandidatosRepresentante($fidfic);
    $datVoc = $votmctele->getCandidatosVocero($fidfic);
} else {
    $datRep = $votmctele->getCandidatosRepresentante(null, $fidcen);
    $datVoc = $votmctele->getCandidatosVocero(null, $fidcen);
}
